...
""" https://github.sre.pub/abcs-ab/Algorithms """
...
# where edges are present without their reversed reflections. We choose the latter approach.
    ...
    """ nx.draw(G, edgelist=all_edges, edge_color=colors,
            with_labels=True, width=1.7)
    plt.show() """
    end = time.time() 
    return {'mincut_edges': m.mincuts_edges, 'mincut_num': m.mincuts_num,  'time': (end - start)}
